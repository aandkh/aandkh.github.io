(function () {
  ...
  const fallbackActors = {
    ...
    5:{id:5,name:'Meryl Streep',profile_path:null,filmCount:2},
    6:{id:6,name:'Matt Damon',profile_path:null,filmCount:2},
    7:{id:7,name:'Anne Hathaway',profile_path:null,filmCount:2},
    8:{id:8,name:'Chris Evans',profile_path:null,filmCount:2},
    9:{id:9,name:'Scarlett Johansson',profile_path:null,filmCount:2},
    10:{id:10,name:'Hugh Jackman',profile_path:null,filmCount:2}
  };
  const fallbackMovies = {
    101:{id:101,title:'Sleepless in Seattle',release_date:'1993-01-01',vote_average:6.8,vote_count:1200,poster_path:null,popularity:20,actors:[1,2]},
    102:{id:102,title:'Catch Me If You Can',release_date:'2002-01-01',vote_average:8.1,vote_count:15000,poster_path:null,popularity:55,actors:[1,3]},
    103:{id:103,title:'Titanic',release_date:'1997-01-01',vote_average:7.9,vote_count:25000,poster_path:null,popularity:70,actors:[3,4]},
    104:{id:104,title:'The Devil Wears Prada',release_date:'2006-01-01',vote_average:7.0,vote_count:9000,poster_path:null,popularity:40,actors:[5,7]},
    105:{id:105,title:'Interstellar',release_date:'2014-01-01',vote_average:8.4,vote_count:36000,poster_path:null,popularity:80,actors:[7,6]},
    106:{id:106,title:'Avengers: Endgame',release_date:'2019-01-01',vote_average:8.3,vote_count:26000,poster_path:null,popularity:90,actors:[8,9]},
    107:{id:107,title:'Movie 43',release_date:'2013-01-01',vote_average:4.6,vote_count:1500,poster_path:null,popularity:18,actors:[10,9]}
  };
  const fallbackPairs = [[1,4],[5,6],[8,10]];
  let fallbackMode = false;

  function fallbackFilmsFor(actorId){
    return Object.values(fallbackMovies).filter(m=>m.actors.includes(actorId));
  }
  ...
  async function searchActors(q) {
    ...
    return withCache('sd:search:' + q.toLowerCase(), 864e5 * 7, async () => {
  ...
  function actorCardHTML(a) {
    ...
    return `<div class="head"><img loading="lazy" src="${src}" alt=""><div><div class="name">${a ? a.name : 'Loading…'}</div><div class="muted">${a ? `${a.filmCount} films` : ''}</div></div></div>`;
  }
  ...
  async function resetRound() {
    ...
    try {
      ...
      if (!a[0] || !b[0]) return setStatus('Could not load actor pair.', true);
  ...
  el.input.addEventListener('input', async (e) => {
    ...
    try {
      ...
      fallbackMode = true;
      const needle=q.toLowerCase();
      state.suggestions=Object.values(fallbackActors).filter(a=>a.name.toLowerCase().includes(needle));
      el.suggestions.innerHTML = state.suggestions.map(s => `<div class="glass suggestion" data-id="${s.id}"><img loading="lazy" src="${NO_FACE}"><div>${s.name}</div></div>`).join('');
      el.useTop.disabled=!state.suggestions.length;
